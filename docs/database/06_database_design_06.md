...
| --- | --- | --- | --- |
| `id` | `SERIAL` | `NOT NULL` | 자동 증가 |
| `camera_id` | `VARCHAR(50)` | `NOT NULL` | 없음 |
...
| `species` | `VARCHAR(50)` | `NOT NULL` | 없음 |
...
| `repeat_detection` | `BOOLEAN` | `NOT NULL` | `FALSE` |
| `repeat_count` | `INTEGER` | `NOT NULL` | `0` |
...
| Event status | `UNCHECKED`, `CHECKING`, `DISPATCH_REQUESTED`, `DISPATCHING`, `COMPLETED`, `MISIDENTIFIED`만 허용 |
| Event bbox | 0~100 퍼센트 좌표, 좌상단 기준. `bbox_x`, `bbox_y`는 0 이상 100 이하, `bbox_width`, `bbox_height`는 0 초과 100 이하 |
| Event priority | 1, 2, 3만 허용. 1=즉시 확인, 2=순차 확인, 3=후순위 확인 |
| Event repeat_count | 0 이상 정수. 0=최초 감지, 1=1회 반복 감지, 2 이상=2회 이상 반복 감지 |
...
CREATE TABLE events (
    ...
    CONSTRAINT ck_events_status
        CHECK (status IN ('UNCHECKED', 'CHECKING', 'DISPATCH_REQUESTED', 'DISPATCHING', 'COMPLETED', 'MISIDENTIFIED')),
    CONSTRAINT ck_events_species
        CHECK (species IN ('gorani', 'wild_boar', 'raccoon')),
    CONSTRAINT ck_events_bbox_x
        CHECK (bbox_x >= 0.0 AND bbox_x <= 100.0),
    CONSTRAINT ck_events_bbox_y
        CHECK (bbox_y >= 0.0 AND bbox_y <= 100.0),
    CONSTRAINT ck_events_bbox_width
        CHECK (bbox_width > 0.0 AND bbox_width <= 100.0),
    CONSTRAINT ck_events_bbox_height
        CHECK (bbox_height > 0.0 AND bbox_height <= 100.0),
    CONSTRAINT ck_events_priority
        CHECK (priority IN (1, 2, 3)),
    CONSTRAINT ck_events_repeat_count
...
| bbox/priority | 프론트 계약에 따라 bbox는 0~100 퍼센트 좌표/좌상단 기준, priority는 1~3 등급으로 저장한다. |
| RoadkillEvent 응답 | DB 컬럼명을 그대로 노출하지 않고 응답 DTO에서 프론트 타입으로 변환한다. |
| AI 탐지 결과 | `obstacle_type`은 `ANIMAL`로 저장하고, 세부 종은 `species`에 저장한다. |
| 반복 감지 | `repeat_detection`, `repeat_count`, `last_detected_at`은 백엔드가 같은 `camera_id`, 같은 `species`, 1분 이상 간격, bbox 중심점 완전 동일 조건으로 판단해 저장한다. |
| priority 격상 | 최초 감지는 `repeat_count=0`, `priority=3`; 1회 반복 감지는 `repeat_count=1`, `priority=2`; 2회 이상 반복 감지는 `repeat_count>=2`, `priority=1`로 저장한다. |
